<template>
  <div
    class="flex flex-col items-center justify-center overflow-visible cursor-pointer hover:bg-bg4 min-w-6"
    data-subtype="menu"
  >
    <div class="flex overflow-visible items-start flex-1 text-xs max-h-4" v-if="col.props.sorting !== 'none'">
      <H_icon
        ...
        class="mx-[-5px]"
        size="1.2rem"
        :color="col.sortDirection.value === 'asc' ? 'pri' : 'txt7'"
      ...
      <H_icon
        ...
        class="mx-[-5px]"
        size="1.2rem"
        :color="col.sortDirection.value === 'desc' ? 'pri' : 'txt7'"
    ...
    <H_icon name="filter" size="1.1rem" :color="col.filter.active ? 'pri' : 'txt7'" v-if="col.filter.type !== 'none'" />
